...
NTSTATUS CalibrateSensor(
	...
	if (devContext->CurrentCapture) { //Cancel current capture
		WdfRequestComplete(devContext->CurrentCapture, STATUS_CANCELLED);
...
CaptureFpData(
	...
	status = WdfRequestRetrieveOutputBuffer(Request, sizeof(WINBIO_CAPTURE_DATA), &CaptureData, &bufferSize);
	if (!NT_SUCCESS(status)) {
		PDWORD payloadSize;
		...
		*payloadSize = sizeof(PWINBIO_CAPTURE_DATA);
		WdfRequestSetInformation(Request, sizeof(DWORD));
		return status;
	}
	...
	if (devContext->CurrentCapture) {
		...
		CaptureData->SensorStatus = sensorMode;
	}
	else {
	...
	WdfRequestSetInformation(Request, sizeof(WINBIO_CAPTURE_DATA));
...
void CompleteFPRequest(
	...
	CaptureData->PayloadSize = sizeof(WINBIO_CAPTURE_DATA);
	CaptureData->WinBioHresult = S_OK;
	CaptureData->CaptureData.Size = 0;

exit:
	WdfRequestComplete(devContext->CurrentCapture, status);
	devContext->CurrentCapture = NULL;
}